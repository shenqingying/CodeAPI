...
using Sonluk.DataAccess.RMS.Utility;

namespace Sonluk.DataAccess.RMS
{
    public static class SQLServerHelper
    {
        public static readonly string connectionString = ConnectionString.Create("RMS", "RMS.DB.User", "RMS.DB.Password", "sVv2QX:GD-");
        ...
        {
            SqlCommand cmd = new SqlCommand();
            cmd.CommandTimeout = 1200;        //在终止执行命令的尝试并生成错误之前的等待时间
        ...
        public static DataSet GetDataSet_dataset(CommandType cmdType, string cmdText, params SqlParameter[] para)
        {
            DataSet ds = new DataSet();
            using (SqlConnection Connection = new SqlConnection(connectionString))
            {
