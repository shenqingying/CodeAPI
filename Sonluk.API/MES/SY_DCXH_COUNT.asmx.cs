...
namespace Sonluk.API.MES
{
    ...
    public class SY_DCXH_COUNT : System.Web.Services.WebService
    {
        ...
        public MES_SY_DCXH_COUNT_SELECT SELECT(MES_SY_DCXH_COUNT model, string ptoken)
        {
            ...
            {
                return mesmodel.SY_DCXH_COUNT.SELECT(model);
            }
            else
            {
        ...
        public MES_RETURN INSERT(MES_SY_DCXH_COUNT model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.SY_DCXH_COUNT.INSERT(model);
            }
            else
            {
        ...
        public MES_RETURN UPDATE(MES_SY_DCXH_COUNT model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.SY_DCXH_COUNT.UPDATE(model);
            }
            else
            {
                MES_RETURN rst = new MES_RETURN();
        ...
        public MES_RETURN DELETE(MES_SY_DCXH_COUNT model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.SY_DCXH_COUNT.DELETE(model);
            }
            else
            {
                MES_RETURN rst = new MES_RETURN();
        ...
        public MES_SY_DCXH_COUNT_SELECT SELECT_LB(MES_SY_DCXH_COUNT model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.SY_DCXH_COUNT.SELECT_LB(model);
            }
            else
            {
                MES_SY_DCXH_COUNT_SELECT rst = new MES_SY_DCXH_COUNT_SELECT();
