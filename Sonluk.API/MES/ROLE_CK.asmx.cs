...
namespace Sonluk.API.MES
{
    ...
    public class ROLE_CK : System.Web.Services.WebService
    {
        MESModels mesmodel = new MESModels();
        RMSModels rmsmodel = new RMSModels();
        [WebMethod]
        public MES_RETURN INSERT(List<MES_ROLE_CK> model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.ROLE_CK.INSERT(model);
            }
            else
            {
        ...
        public MES_RETURN DELETE(int ROLEID, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.ROLE_CK.DELETE(ROLEID);
            }
            else
            {
                MES_RETURN rst = new MES_RETURN();
        ...
        public MES_ROLE_CK_LIST SELECT(int ROLEID, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.ROLE_CK.SELECT(ROLEID);
            }
            else
            {
