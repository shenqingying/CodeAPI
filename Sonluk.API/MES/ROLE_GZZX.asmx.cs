...
namespace Sonluk.API.MES
{
    ...
    public class ROLE_GZZX : System.Web.Services.WebService
    {
        MESModels mesmodel = new MESModels();
        RMSModels rmsmodel = new RMSModels();
        [WebMethod]
        public MES_RETURN INSERT(List<MES_SY_GZZX_LAY> model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.ROLE_GZZX.INSERT(model);
            }
            else
            {
        ...
        public MES_RETURN DELETE(int ROLEID, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.ROLE_GZZX.DELETE(ROLEID);
            }
            else
            {
                MES_RETURN rst = new MES_RETURN();
        ...
        public MES_ROLE_GZZX_SELECT_BYROLEID SELECT_BYROLEID(int ROLEID, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.ROLE_GZZX.SELECT_BYROLEID(ROLEID);
            }
            else
            {
