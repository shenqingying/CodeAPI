...
namespace Sonluk.API.MES
{
    ...
    public class WS_MES_SY_LANGUAGE_YM : System.Web.Services.WebService
    {
        MESModels mesmodel = new MESModels();
        RMSModels rmsmodel = new RMSModels();
        [WebMethod]
        public MES_RETURN INSERT(MES_SY_LANGUAGE_YM model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                return mesmodel.SY_LANGUAGE_YM.INSERT(model, 0);
            }
            else
            {
        ...
        public MES_SY_LANGUAGE_TABLEZD_SELECT SELECT_TABLEZD(MES_SY_LANGUAGE_TABLEZD model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                ...
                model.SYLANGUAGEID = tokeninfo.LANGUAGEID;
                MES_SY_LANGUAGE_TABLEZD_SELECT rst = mesmodel.SY_LANGUAGE_YM.SELECT_TABLEZD(model);
        ...
        [WebMethod]
        public MES_SY_LANGUAGE_ZD_SELECT SELECT_ZD(MES_SY_LANGUAGE_ZD model, string ptoken)
        {
            if (rmsmodel.CRM_Login.ValidateToken(ptoken))
            {
                TokenInfo tokeninfo = rmsmodel.CRM_Login.get_ptokeninfo_language(ptoken);
                model.SYLANGUAGEID = tokeninfo.LANGUAGEID;
                MES_SY_LANGUAGE_ZD_SELECT rst = mesmodel.SY_LANGUAGE_YM.SELECT_ZD(model);
                rst.MES_RETURN = mesmodel.SY_LANGUAGE_RETURN.RETURN_MESSAGE(rst.MES_RETURN, ptoken);
                return rst;
            }
            else
